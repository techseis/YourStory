...
namespace YStory.Core.Plugins
{
    ...
    public class PluginManager
    {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(YStoryVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        private static IEnumerable<KeyValuePair<FileInfo, PluginDescriptor>> GetDescriptionFilesAndDescriptors(DirectoryInfo pluginFolder)
        {
            ...
            //add display subscription and path to list
            ...
            //sort list by display subscription. NOTE: Lowest DisplaySubscription will be first i.e 0 , 1, 1, 1, 5, 10
            //it's required: http://www.yourstory.com/boards/t/17455/load-plugins-based-on-their-displaysubscription-on-startup.aspx
            result.Sort((firstPair, nextPair) => firstPair.Value.DisplaySubscription.CompareTo(nextPair.Value.DisplaySubscription));
            return result;
        }
        ...
        private static Assembly PerformFileDeploy(FileInfo plug)
        {
            if (plug.Directory == null || plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name + " file exists in a folder outside of the allowed yourStory folder hierarchy");
        ...
        private static FileInfo InitializeMediumTrust(FileInfo plug, DirectoryInfo shadowCopyPlugFolder)
        {
            ...
            if (shadowCopiedPlug.Exists)
            {
                ...
                else
                {
                    ...
                    //More info: http://www.yourstory.com/boards/t/11511/access-error-nopplugindiscountrulesbillingcountrydll.aspx?p=4#60838
