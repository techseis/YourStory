...
namespace YStory.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        /// {0} : category ID
        /// </remarks>
        private const string CATEGORIES_BY_ID_KEY = "YStory.category.id-{0}";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        private const string CATEGORIES_BY_PARENT_CATEGORY_ID_KEY = "YStory.category.byparent-{0}-{1}-{2}-{3}-{4}";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        ...
        /// {5} : store ID
        /// </remarks>
        private const string ARTICLECATEGORIES_ALLBYCATEGORYID_KEY = "YStory.articlecategory.allbycategoryid-{0}-{1}-{2}-{3}-{4}-{5}";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        ...
        /// {3} : store ID
        /// </remarks>
        private const string ARTICLECATEGORIES_ALLBYARTICLEID_KEY = "YStory.articlecategory.allbyarticleid-{0}-{1}-{2}-{3}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string CATEGORIES_PATTERN_KEY = "YStory.category.";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string ARTICLECATEGORIES_PATTERN_KEY = "YStory.articlecategory.";
        ...
        /// <param name="articleCategoryRepository">ArticleCategory repository</param>
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            else
            {
                ...
                query = query.OrderBy(c => c.ParentCategoryId).ThenBy(c => c.DisplaySubscription).ThenBy(c => c.Id);
            ...
            bool showHidden = false, bool includeAllLevels = false)
        {
            ...
            return _cacheManager.Get(key, () =>
            {
                ...
                query = query.Where(c => !c.Deleted);
                query = query.OrderBy(c => c.DisplaySubscription).ThenBy(c => c.Id);
        ...
        public virtual IList<Category> GetAllCategoriesDisplayedOnHomePage(bool showHidden = false)
        {
            var query = from c in _categoryRepository.Table
                        orderby c.DisplaySubscription, c.Id
                        where c.Published &&
                        !c.Deleted && 
                        c.ShowOnHomePage
                        select c;
        ...
        public virtual void DeleteArticleCategory(ArticleCategory articleCategory)
        {
            if (articleCategory == null)
                throw new ArgumentNullException("articleCategory");
            ...
            //event notification
            _eventPublisher.EntityDeleted(articleCategory);
        }
        ...
        public virtual IPagedList<ArticleCategory> GetArticleCategoriesByCategoryId(int categoryId,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            if (categoryId == 0)
                return new PagedList<ArticleCategory>(new List<ArticleCategory>(), pageIndex, pageSize);

            string key = string.Format(ARTICLECATEGORIES_ALLBYCATEGORYID_KEY, showHidden, categoryId, pageIndex, pageSize, _workContext.CurrentCustomer.Id, _storeContext.CurrentStore.Id);
            return _cacheManager.Get(key, () =>
            {
                var query = from pc in _articleCategoryRepository.Table
                            join p in _articleRepository.Table on pc.ArticleId equals p.Id
                            where pc.CategoryId == categoryId &&
                                  !p.Deleted &&
                                  (showHidden || p.Published)
                            orderby pc.DisplaySubscription, pc.Id
                            select pc;
                ...
                if (!showHidden && (!_catalogSettings.IgnoreAcl || !_catalogSettings.IgnoreStoreLimitations))
                {
                    ...
                    query = from c in query
                            ...
                            orderby cGroup.Key
                            select cGroup.FirstOrDefault();
                    query = query.OrderBy(pc => pc.DisplaySubscription).ThenBy(pc => pc.Id);
                }

                var articleCategories = new PagedList<ArticleCategory>(query, pageIndex, pageSize);
        ...
        public virtual IList<ArticleCategory> GetArticleCategoriesByArticleId(int articleId, bool showHidden = false)
        {
            return GetArticleCategoriesByArticleId(articleId, _storeContext.CurrentStore.Id, showHidden);
        }
        /// <summary>
        /// Gets a article category mapping collection
        /// </summary>
        ...
        /// <param name="showHidden"> A value indicating whether to show hidden records</param>
        /// <returns> Article category mapping collection</returns>
        public virtual IList<ArticleCategory> GetArticleCategoriesByArticleId(int articleId, int storeId, bool showHidden = false)
        {
            if (articleId == 0)
                return new List<ArticleCategory>();

            string key = string.Format(ARTICLECATEGORIES_ALLBYARTICLEID_KEY, showHidden, articleId, _workContext.CurrentCustomer.Id, storeId);
            return _cacheManager.Get(key, () =>
            {
                var query = from pc in _articleCategoryRepository.Table
                            join c in _categoryRepository.Table on pc.CategoryId equals c.Id
                ...
                var allArticleCategories = query.ToList();
                var result = new List<ArticleCategory>();
                if (!showHidden)
                {
                    foreach (var pc in allArticleCategories)
                    {
                ...
                {
                    //no filtering
                    result.AddRange(allArticleCategories);
                }
                return result;
            });
        }
        ...
        public virtual ArticleCategory GetArticleCategoryById(int articleCategoryId)
        {
            if (articleCategoryId == 0)
                return null;

            return _articleCategoryRepository.GetById(articleCategoryId);
        }
        ...
        public virtual void InsertArticleCategory(ArticleCategory articleCategory)
        {
            if (articleCategory == null)
                throw new ArgumentNullException("articleCategory");
            ...
            //event notification
            _eventPublisher.EntityInserted(articleCategory);
        }
        ...
        /// </summary>
        /// <param name="articleCategory">>Article category mapping</param>
        public virtual void UpdateArticleCategory(ArticleCategory articleCategory)
        {
            if (articleCategory == null)
                throw new ArgumentNullException("articleCategory");
            ...
            //event notification
            _eventPublisher.EntityUpdated(articleCategory);
        }
        ...
        public virtual IDictionary<int, int[]> GetArticleCategoryIds(int[] articleIds)
        {
            var query = _articleCategoryRepository.Table;

            return query.Where(p => articleIds.Contains(p.ArticleId))
                .Select(p => new {p.ArticleId, p.CategoryId}).ToList()
