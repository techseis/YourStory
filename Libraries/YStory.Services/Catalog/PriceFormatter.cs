...
namespace YStory.Services.Catalog
{
    ...
    public partial class PriceFormatter : IPriceFormatter
    {
        ...
        /// <summary>
        /// Formats the price of rental article (with rental period)
        /// </summary>
        /// <param name="article">Article</param>
        /// <param name="price">Price</param>
        /// <returns>Rental article price with period</returns>
        public virtual string FormatRentalArticlePeriod(Article article, string price)
        {
            if (article == null)
            ...
            switch (article.RentalPricePeriod)
            {
                case RentalPricePeriod.Days:
                    result = string.Format(_localizationService.GetResource("Articles.Price.Rental.Days"), price, article.RentalPriceLength);
                    break;
                case RentalPricePeriod.Weeks:
                    result = string.Format(_localizationService.GetResource("Articles.Price.Rental.Weeks"), price, article.RentalPriceLength);
                    break;
                case RentalPricePeriod.Months:
                    result = string.Format(_localizationService.GetResource("Articles.Price.Rental.Months"), price, article.RentalPriceLength);
                    break;
                case RentalPricePeriod.Years:
                    result = string.Format(_localizationService.GetResource("Articles.Price.Rental.Years"), price, article.RentalPriceLength);
                    break;
                default:
                    throw new YStoryException("Not supported rental period");
            }
