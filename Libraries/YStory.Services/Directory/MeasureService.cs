...
namespace YStory.Services.Directory
{
    /// <summary>
    ...
    public partial class MeasureService : IMeasureService
    {
        ...
        /// Key for caching
        /// </summary>
        private const string MEASUREDIMENSIONS_ALL_KEY = "YStory.measuredimension.all";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        /// {0} : dimension ID
        /// </remarks>
        private const string MEASUREDIMENSIONS_BY_ID_KEY = "YStory.measuredimension.id-{0}";
        /// <summary>
        /// Key for caching
        /// </summary>
        private const string MEASUREWEIGHTS_ALL_KEY = "YStory.measureweight.all";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        /// {0} : weight ID
        /// </remarks>
        private const string MEASUREWEIGHTS_BY_ID_KEY = "YStory.measureweight.id-{0}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string MEASUREDIMENSIONS_PATTERN_KEY = "YStory.measuredimension.";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string MEASUREWEIGHTS_PATTERN_KEY = "YStory.measureweight.";
        ...
        public virtual IList<MeasureDimension> GetAllMeasureDimensions()
        {
            ...
            return _cacheManager.Get(key, () =>
            {
                var query = from md in _measureDimensionRepository.Table
                            orderby md.DisplaySubscription, md.Id
                            select md;
            ...
            MeasureDimension sourceMeasureDimension)
        {
            ...
            if (result != decimal.Zero && sourceMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new YStoryException(string.Format("Exchange ratio not set for dimension [{0}]", sourceMeasureDimension.Name));
            ...
            MeasureDimension targetMeasureDimension)
        {
            ...
            if (result != decimal.Zero && targetMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new YStoryException(string.Format("Exchange ratio not set for dimension [{0}]", targetMeasureDimension.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
        ...
        public virtual IList<MeasureWeight> GetAllMeasureWeights()
        {
            ...
            return _cacheManager.Get(key, () =>
            {
                var query = from mw in _measureWeightRepository.Table
                            orderby mw.DisplaySubscription, mw.Id
                            select mw;
        ...
        public virtual decimal ConvertToPrimaryMeasureWeight(decimal value, MeasureWeight sourceMeasureWeight)
        {
            ...
            if (result != decimal.Zero && sourceMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new YStoryException(string.Format("Exchange ratio not set for weight [{0}]", sourceMeasureWeight.Name));
                result = result / exchangeRatio;
            }
            return result;
        }
            ...
            MeasureWeight targetMeasureWeight)
        {
            ...
            if (result != decimal.Zero && targetMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new YStoryException(string.Format("Exchange ratio not set for weight [{0}]", targetMeasureWeight.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
