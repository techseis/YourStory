...
namespace YStory.Services.Directory
{
    ...
    public partial class StateProvinceService : IStateProvinceService
    {
        ...
        /// </remarks>
        private const string STATEPROVINCES_ALL_KEY = "YStory.stateprovince.all-{0}-{1}-{2}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string STATEPROVINCES_PATTERN_KEY = "YStory.stateprovince.";
        ...
        public virtual IList<StateProvince> GetStateProvincesByCountryId(int countryId, int languageId = 0, bool showHidden = false)
        {
            ...
            {
                var query = from sp in _stateProvinceRepository.Table
                            orderby sp.DisplaySubscription, sp.Name
                ...
                if (languageId > 0)
                {
                    //we should sort states by localized names when they have the same display subscription
                    stateProvinces = stateProvinces
                        .OrderBy(c => c.DisplaySubscription)
        ...
        {
            var query = from sp in _stateProvinceRepository.Table
                        orderby sp.CountryId, sp.DisplaySubscription, sp.Name
