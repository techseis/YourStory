...
namespace YStory.Services.ExportImport.Help
{
    /// <summary>
    ...
    public class PropertyManager<T>
    {
        ...
        public PropertyManager(IEnumerable<PropertyByName<T>> properties)
        {
            ...
            {
                propertyByName.PropertySubscriptionPosition = poz;
                poz++;
        ...
        public int GetIndex(string propertyName)
        {
            ...
            return _properties[propertyName].PropertySubscriptionPosition;
        }
        ...
        public void WriteToXlsx(ExcelWorksheet worksheet, int row, bool exportImportUseDropdownlistsForAssociatedEntities, int cellOffset = 0, ExcelWorksheet fWorksheet=null)
        {
            ...
            {
                var cell = worksheet.Cells[row, prop.PropertySubscriptionPosition + cellOffset];
                if (prop.IsDropDownCell)
                {
                    ...
                    {
                        var fCell = fWorksheet.Cells[fRow++, prop.PropertySubscriptionPosition];
                    ...
                    validator.Formula.ExcelFormula = string.Format("{0}!{1}:{2}", fWorksheet.Name, fWorksheet.Cells[1, prop.PropertySubscriptionPosition].Address, fWorksheet.Cells[dropDownElements.Length, prop.PropertySubscriptionPosition].Address);
                }
                else
                {
        ...
        public void ReadFromXlsx(ExcelWorksheet worksheet, int row, int cellOffset = 0)
        {
            ...
            foreach (var prop in _properties.Values)
            {
                prop.PropertyValue = worksheet.Cells[row, prop.PropertySubscriptionPosition + cellOffset].Value;
            }
        }
        ...
        public void WriteCaption(ExcelWorksheet worksheet, Action<ExcelStyle> setStyle, int row = 1, int cellOffset = 0)
        {
            ...
            {
                var cell = worksheet.Cells[row, caption.PropertySubscriptionPosition + cellOffset];
                cell.Value = caption;
                setStyle(cell.Style);
            }
        }
