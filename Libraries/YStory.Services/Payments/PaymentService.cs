...
namespace YStory.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
        ...
        /// Process a payment
        /// </summary>
        /// <param name="processPaymentRequest">Payment info required for an subscription processing</param>
        /// <returns>Process payment result</returns>
        public virtual ProcessPaymentResult ProcessPayment(ProcessPaymentRequest processPaymentRequest)
        {
            if (processPaymentRequest.SubscriptionTotal == decimal.Zero)
            {
        ...
        /// </summary>
        /// <param name="postProcessPaymentRequest">Payment info required for an subscription processing</param>
        public virtual void PostProcessPayment(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            //already paid or subscription.SubscriptionTotal == decimal.Zero
            if (postProcessPaymentRequest.Subscription.PaymentStatus == PaymentStatus.Paid)
                return;

            var paymentMethod = LoadPaymentMethodBySystemName(postProcessPaymentRequest.Subscription.PaymentMethodSystemName);
            if (paymentMethod == null)
                throw new YStoryException("Payment method couldn't be loaded");
        ...
        /// <summary>
        /// Gets a value indicating whether customers can complete a payment after subscription is placed but not completed (for redirection payment methods)
        /// </summary>
        /// <param name="subscription">Subscription</param>
        /// <returns>Result</returns>
        public virtual bool CanRePostProcessPayment(Subscription subscription)
        {
            ...
            var paymentMethod = LoadPaymentMethodBySystemName(subscription.PaymentMethodSystemName);
            if (paymentMethod == null)
            ...
            if (subscription.Deleted)
                return false;  //do not allow for deleted subscriptions

            if (subscription.SubscriptionStatus == SubscriptionStatus.Cancelled)
                return false;  //do not allow for cancelled subscriptions

            if (subscription.PaymentStatus != PaymentStatus.Pending)
                return false;  //payment status should be Pending

            return paymentMethod.CanRePostProcessPayment(subscription);
        }
        ...
        {
            var paymentMethod = LoadPaymentMethodBySystemName(capturePaymentRequest.Subscription.PaymentMethodSystemName);
            if (paymentMethod == null)
                throw new YStoryException("Payment method couldn't be loaded");
        ...
        {
            var paymentMethod = LoadPaymentMethodBySystemName(refundPaymentRequest.Subscription.PaymentMethodSystemName);
            if (paymentMethod == null)
                throw new YStoryException("Payment method couldn't be loaded");
        ...
        {
            var paymentMethod = LoadPaymentMethodBySystemName(voidPaymentRequest.Subscription.PaymentMethodSystemName);
            if (paymentMethod == null)
                throw new YStoryException("Payment method couldn't be loaded");
        ...
        {
            if (cancelPaymentRequest.Subscription.SubscriptionTotal == decimal.Zero)
                return new CancelRecurringPaymentResult();

            var paymentMethod = LoadPaymentMethodBySystemName(cancelPaymentRequest.Subscription.PaymentMethodSystemName);
            if (paymentMethod == null)
                throw new YStoryException("Payment method couldn't be loaded");
