...
namespace YStory.Services.Subscriptions
{
    ...
    public partial class ReturnRequestService : IReturnRequestService
    {
        ...
        /// <param name="subscriptionItemId">Subscription item identifier; 0 to load all entries</param>
        ...
        public IPagedList<ReturnRequest> SearchReturnRequests(int storeId = 0, int customerId = 0,
            int subscriptionItemId = 0, string customNumber = "", ReturnRequestStatus? rs = null,  DateTime? createdFromUtc = null,
            DateTime? createdToUtc = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (subscriptionItemId > 0)
                query = query.Where(rr => rr.SubscriptionItemId == subscriptionItemId);
        ...
        public virtual IList<ReturnRequestAction> GetAllReturnRequestActions()
        {
            var query = from rra in _returnRequestActionRepository.Table
                orderby rra.DisplaySubscription, rra.Id
                select rra;
            return query.ToList();
        }
