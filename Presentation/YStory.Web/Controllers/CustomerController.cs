...
using YStory.Services.Authentication;
...
using YStory.Web.Framework.Security.Honeypot;
...
namespace YStory.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    if (_customerSettings.NewsletterEnabled)
                    {
                        ...
                        var newsletter = _newsLetterSubscriptionService.GetNewsLetterOrderByEmailAndStoreId(model.Email, _storeContext.CurrentStore.Id);
                        if (newsletter != null)
                        {
                            if (model.Newsletter)
                            {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (_customerSettings.NewsletterEnabled)
                    {
                        //save newsletter value
                        var newsletter =
                            _newsLetterSubscriptionService.GetNewsLetterOrderByEmailAndStoreId(customer.Email,
                                _storeContext.CurrentStore.Id);
                        if (newsletter != null)
                        {
                            if (model.Newsletter)
        ...
        #region My account / Downloadable articles

      

        public virtual ActionResult UserAgreement(Guid subscriptionItemId)
        {
            var subscriptionItem = _subscriptionService.GetSubscriptionItemByGuid(subscriptionItemId);
            if (subscriptionItem == null)
                return RedirectToRoute("HomePage");

            var article = subscriptionItem.Article;
            if (article == null || !article.HasUserAgreement)
                return RedirectToRoute("HomePage");

            var model = _customerModelFactory.PrepareUserAgreementModel(subscriptionItem, article);
            return View(model);
        }
                ...
                try
                {
                    ...
                    if ((uploadedFile != null) && (!String.IsNullOrEmpty(uploadedFile.FileName)))
                    {
                        ...
                        if (uploadedFile.ContentLength > avatarMaxSize)
                            throw new YStoryException(string.Format(_localizationService.GetResource("Account.Avatar.MaximumUploadedFileSize"), avatarMaxSize));
